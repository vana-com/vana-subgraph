...
describe("handleDataRegistryProofAddedV2", () => {
  test("creates proof, totals, and user totals", () => {
    ...
    handleDataRegistryProofAddedV2(proofEvent);
  ...
  test("fails when current epoch reference is not set", () => {
    const proofEvent = createProofAddedEvent(1, 0, 1, 100);
  ...
  test("fails when dlp does not exist", () => {
    createNewEpochReference(EPOCH_REFERENCE_ID_CURRENT, "1");
    const proofEvent = createProofAddedEvent(1, 0, 999, 100);
...
describe("handleFileAddedV2", () => {
  test("creates file owner entity", () => {
    ...
    handleFileAddedV2(fileAddedEvent);
