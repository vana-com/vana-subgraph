...
import { createFileFromEvent, logDataRegistryEvent } from "../../../../../src/lib/contract/shared/file-handlers";
...
describe("createFileFromEvent", () => {
  test("creates a File entity with all required fields", () => {
    ...
    assert.fieldEquals("File", fileId, "addedAtBlock", mockBlock.number.toString());
    assert.fieldEquals("File", fileId, "addedAtTimestamp", mockBlock.timestamp.toString());
    assert.fieldEquals("File", fileId, "transactionHash", mockTransaction.hash.toHexString());
