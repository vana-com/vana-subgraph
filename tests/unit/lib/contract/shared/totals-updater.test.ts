...
describe("updateGlobalTotals", () => {
  test("updates global totals for first-time user", () => {
    ...
    // Check global totals
    assert.fieldEquals("Totals", TOTALS_ID_GLOBAL, "totalFileContributions", "1");
    assert.fieldEquals("Totals", TOTALS_ID_GLOBAL, "uniqueFileContributors", "1");
  });
  ...
  test("updates global totals for returning user", () => {
    ...
    assert.fieldEquals("Totals", TOTALS_ID_GLOBAL, "totalFileContributions", "1");
    assert.fieldEquals("Totals", TOTALS_ID_GLOBAL, "uniqueFileContributors", "0");
  });
});
...
describe("updateDlpTotals", () => {
  test("updates DLP totals for first-time user", () => {
    ...
    assert.entityCount("UserTotals", 1);
    assert.fieldEquals("UserTotals", dlpUserTotalsId, "fileContributionsCount", "1");
...
describe("updateTotalsFromFile", () => {
  ...
  test("handles non-existent file gracefully", () => {
    ...
    assert.fieldEquals("Totals", TOTALS_ID_GLOBAL, "totalFileContributions", "0");
    assert.fieldEquals("Totals", TOTALS_ID_GLOBAL, "uniqueFileContributors", "0");
  });
});