...
export function getEthPriceInUSD(): BigDecimal {
  ...
  const usdcPool = Pool.load(Address.fromString(stablePool)); // dai is token0
  if (usdcPool) {
...
export function getTrackedAmountUSD(
  ...
  if (
    WHITELIST_TOKENS.includes(token0.id.toHexString()) &&
    WHITELIST_TOKENS.includes(token1.id.toHexString())
  ) {
